...
angular.module('networkEventGraphApp')
    .controller('TimespanPickerCtrl', function ($scope, $timeout) {
        var date = new Date();
        date.setHours(0);
        date.setMinutes(0);
        ...
        var setPeriodParams = function() {
            ...
            if ($scope.period.fromDate !== undefined) {
                $scope.sessionParams.start = ($scope.period.fromDate.getTime() / 1000.0).toFixed(0) + ftime;
            }
            if ($scope.period.toDate !== undefined) {
                $scope.sessionParams.end = ($scope.period.toDate.getTime() / 1000.0).toFixed(0) + ttime;
            }
        };
